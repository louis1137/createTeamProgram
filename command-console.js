	...
	executeCommand() {
		...
		if (this.inputMode === 'delete-password-confirm') {
			...
			if (cmd === this.storedPassword) {
				...
				this.warn(`⚠️ ${this.comments.profileDeleteQuestion.replace('프로필을', `프로필 '${currentRoomKey}'를`)}`);
		...
		if (this.inputMode === 'delete-final-confirm') {
			...
			if (cmd === currentRoomKey) {
				...
				database.ref(`rooms/${currentRoomKey}`).remove()
					.then(() => {
						this.success(`🗑️ ${this.comments.profileDeleted.replace('프로필이', `프로필 '${currentRoomKey}'가`).replace('삭제되었습니다', '완전히 삭제되었습니다')}`);
